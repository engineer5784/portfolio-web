...
export default function chatbot() {
  ...
  const onSubmit = async (data, e) => {
    ...
    try {
      await emailjs.send(
        'service_software', // ID del servicio en EmailJS
        'template_v0z4jfa', // ID plantilla en EmailJS
        {
          user_name: data.name,
          message: data.message,
          user_email: data.email,
        },
        'VqR0XfnLloJW41nyF' // ID de usuario en EmailJS
      );
  
      setIsSuccess(true);
      setMessage('¡El mensaje se envió con éxito!');
      e.target.reset();
      reset();
    } catch (error) {
      console.error('Error sending email:', error);
      setIsSuccess(false);
      setMessage('Ocurrió un error al enviar el mensaje. Por favor, inténtalo de nuevo más tarde.');
    }
  };
